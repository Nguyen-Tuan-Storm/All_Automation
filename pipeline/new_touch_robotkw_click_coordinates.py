...
class _TouchKeywords(KeywordGroup):
    ...
    def click_element_at_coordinates(self, coordinate_X, coordinate_Y):
        """Nhấn vào tọa độ cụ thể bằng cách sử dụng PointerInput trong Selenium 4."""
        driver = self._current_application()
        # Tạo đối tượng PointerInput để xử lý sự kiện cảm ứng hoặc chuột
        touch_input = PointerInput("touch", "touch")

        # Tạo ActionBuilder instance
        actions = ActionBuilder(driver, mouse=touch_input)

        # Di chuyển đến tọa độ cụ thể và thực hiện thao tác nhấn và thả
        actions.pointer_action.move_to_location(coordinate_X, coordinate_Y)
